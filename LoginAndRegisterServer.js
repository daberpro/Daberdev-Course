...
function GoogleSaveToDatabase(profile,cb) {
    ...
    db.UserModel.findOne({
        ...
        if (!res) {
            await save_order_history().catch(d =>{
                profile.status = "failed"
                console.log(d);            
            });
            await save_user().catch(d =>{
                profile.status = "failed"
                console.log(d);
            });
            cb(null, profile);
...
passport.use(new GoogleStrategy({
        ...
        callbackURL: `http://daberdev.id:${process.env.AUTH_PORT}/auth/google/callback`
    },
...
app.get('/auth/google/callback',
    ...
    function(req, res) {
        ...
        if(req.user.status === "failed") return res.redirect(`http://daberdev.id:${process.env.MAIN_PORT}/login`);
        res.redirect(`http://daberdev.id:${process.env.MAIN_PORT}/profile?token=${req.user.tokens.token}&ref_token=${req.user.tokens.refreshToken}`);
    });
...
passport.use(new GithubStrategy({
        ...
        callbackURL: `http://daberdev.id:${process.env.AUTH_PORT}/auth/github/callback`
    },
    function(accessToken, refreshToken, profile, cb) {
        // console.log(profile)
        // cb(null,profile);
