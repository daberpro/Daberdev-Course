...
<div class="items">
	<div class="list">
		{#if items.length === 0}
			<h2 style="color: white;margin: 0px;padding: 0px; margin-top: 40%;">Nothing here..</h2>
			<span style="color: gray;">please add some item to checkout</span>
		{/if}
		{#each items as data, i}
			<div class="item">
				<div style={`background: ${data.bgColor}`} class="thumbnail">
				</div>
				<b>{data.title}</b>
				<span>
		...
		<button style={total > 0? '' : `filter:blur(3px)`} primary on:click={async _=>{
			if(total !== 0){
				...
				if(response.ok){
					...
					// console.log(result)
					localStorage.setItem("simple-shop-token",result.token)
